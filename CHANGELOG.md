...
- **Member self-service portal** — Passwordless **Membership** profile at [`membership.php`](membership.php) (`/membership`): email a one-time magic link, open a short member session, and immediately update phone/address, emergency contact, AMA/FAA (with Verify AMA), badge photo, FAA card, and email preferences. Primary email and membership status stay read-only. Saves write `audit_log` (`member_self_update`) and notify the Installation **Membership email** (fallback: Support email, then first active admin). AMA/FAA expiry reminders link to the durable `/membership` page (email prefilled); the one-hour magic link is issued only when the member requests access. Staff can **Send profile link** from member edit. Migration: [`scripts/migrate_member_portal.sql`](scripts/migrate_member_portal.sql). Docs: [docs/members.html#self-service](docs/members.html#self-service).
